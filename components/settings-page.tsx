...
import { AlertCircle, CheckCircle2, CirclePlus, Clock3, Lightbulb, MapPin, Save, Trash2, Zap } from "lucide-react";
...
type AppSettings = {
  ...
  absoluteToleranceKwh: string;
  closedFromHour: number;
  closedToHour: number;
  updatedAt: string;
};
...
export function SettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [tariffData, settingsData] = await Promise.all([
        fetch("/api/tariffs", { cache: "no-store" }).then(responseJson<{ tariffs: Tariff[] }>),
        fetch("/api/settings", { cache: "no-store" }).then(responseJson<{ settings: AppSettings }>),
      ]);
  ...
  const currencyCode = settings?.currencyCode ?? "RUB";
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await fetch("/api/settings", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(settings) }).then(responseJson<{ settings: AppSettings }>);
      setSettings(data.settings);
      setNotice("Параметры объекта сохранены и применены к новым расчётам.");
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "Не удалось сохранить параметры объекта");
  ...
  return <>
    ...
    <section className="surface-card overflow-hidden">
      <div className="border-b border-[#e6edef] p-5 sm:p-6"><h2 className="section-title">Параметры объекта</h2><p className="mt-1 text-sm text-[#738792]">Эти значения используются сервером независимо от часового пояса браузера.</p></div>
      {settings ? <form className="grid gap-4 p-5 sm:p-6" onSubmit={saveSettings}>
        <div className="grid gap-4 lg:grid-cols-3"><label className="grid gap-2 text-sm font-medium text-[#29475a]">Название объекта<input className={fieldClass} maxLength={120} value={settings.facilityName} onChange={event => setSettings({ ...settings, facilityName: event.target.value })} required /></label><label className="grid gap-2 text-sm font-medium text-[#29475a]">Часовой пояс IANA<input className={fieldClass} placeholder="Europe/Moscow" value={settings.timezone} onChange={event => setSettings({ ...settings, timezone: event.target.value })} required /></label><label className="grid gap-2 text-sm font-medium text-[#29475a]">Валюта<input className={fieldClass} maxLength={3} placeholder="RUB" value={settings.currencyCode} onChange={event => setSettings({ ...settings, currencyCode: event.target.value.toUpperCase() })} required /></label></div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label className="grid gap-2 text-sm font-medium text-[#29475a]">Процентный допуск, %<input className={fieldClass} inputMode="decimal" value={settings.percentageTolerance} onChange={event => setSettings({ ...settings, percentageTolerance: event.target.value })} required /></label><label className="grid gap-2 text-sm font-medium text-[#29475a]">Абсолютный допуск, кВт⋅ч<input className={fieldClass} inputMode="decimal" value={settings.absoluteToleranceKwh} onChange={event => setSettings({ ...settings, absoluteToleranceKwh: event.target.value })} required /></label><label className="grid gap-2 text-sm font-medium text-[#29475a]">Закрыто с<select className={fieldClass} value={settings.closedFromHour} onChange={event => setSettings({ ...settings, closedFromHour: Number(event.target.value) })}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label><label className="grid gap-2 text-sm font-medium text-[#29475a]">Закрыто до<select className={fieldClass} value={settings.closedToHour} onChange={event => setSettings({ ...settings, closedToHour: Number(event.target.value) })}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label></div>
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-[#718590]">Период закрытия используется для расчёта ночной нагрузки.</p><Button className="rounded-xl bg-[#153d59] text-white" disabled={saving} type="submit"><Save size={16} />{saving ? "Сохраняем…" : "Сохранить параметры"}</Button></div>
      </form> : <p className="p-6 text-sm text-[#718590]">Загрузка параметров…</p>}
    </section>
