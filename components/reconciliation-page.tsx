...
type Row = { date: string; hour: number; actualKwh: string | null; quality: string; devicesKwh: string; deltaKwh: string | null; deltaPercent: number | null; status: string; errorMessage?: string; details: Detail[] };
type Reconciliation = {
...
export function ReconciliationPage() {
  ...
  function exportCsv() {
    ...
    const header = ["Дата", "Час", "Факт, кВт⋅ч", "Качество", "Приборы, кВт⋅ч", "Разница, кВт⋅ч", "Разница, %", "Статус", "Приборы и формулы"];
    const lines = filteredRows.map(row => [row.date, `${String(row.hour).padStart(2, "0")}:00`, row.actualKwh ?? "", row.quality, row.devicesKwh, row.deltaKwh ?? "", row.deltaPercent?.toFixed(2) ?? "", statusLabel(row.status), row.details.map(detail => `${detail.name}: ${detail.formula} = ${detail.energyKwh}`).join(" | ")]);
